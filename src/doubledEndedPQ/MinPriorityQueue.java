...
/**
 * This class implements a priority queue as a binary heap. Highest priority is the smallest element.
 * 
 * Note: For proper implementation of this class by the double-ended priority queue, all updates made to
 * 'heap' must also be made in parallel to 'maxHeap.heap'
 * 
...
public class MinPriorityQueue<E extends Comparable<? super E>>
{
     ...
     * of children that each node will have.) Priority is specified using the natural order of the objects.
     * Objects that are 'less than' other objects are first (have higher priority.)
     */
    ...
    public void add (E element)
    {
        ...
        while((element.compareTo(parent) == -1) && (currentIndex != 0))
        {
    ...
    public E poll ()
    {
        ...
        while(true)
        {
	        ...
	        for(int child = 1; child < width; child++)
	        {
	            ...
	            if(currentChildElement.compareTo(hiPriElement) == -1)
	            {	
     ...
     * the double-ended priority queue, we can assume that the parameter element E is the largest element in
     * the corresponding maxPriorityQueue (and thus the largest element in the double-ended priority queue). 
     * 
     * @param E the element to be removed (max element)
     */
     ...
     * 
     * @return E the element with the smallest value according to natural ordering
     */
    public E peek()
    {
    	if(heap.size() == 0)	// it heap is empty
