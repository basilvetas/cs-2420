...
public class Experiment3 
{
	...
	{
		new Experiment3().testQuicksort();	// initiates the tests
	}
		...
		System.out.println("Array Size: \tAverage Comparisons: "); // print data labels
			...
			for(int arraySize = startArraySize1; arraySize < endArraySize1+1; arraySize++) // for each array size
			{
				totalComparisons = 0;
					...
					{
						double[] randomData = createArrayOfSize10s(arraySize); // create a random array
	 ...
	 */
	public static double[] createArrayOfSize10s(int size)
	{
		double[] data = new double[size];
		
		for(int i = 0; i < data.length; i++)	// for each position
		{
			int randomValue = (int) (10*Math.random());	// create a random value
	...
	public int partition (double[] data, int start, int end)
	{
	    ...
	    while(true)
	    {
	    	...
	    	while(advanceCount(right > left && data[right] > pivot))	// changed for question three (>= changed to >)
	    		right = right - 1;
