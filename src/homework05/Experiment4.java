...
public class Experiment4 
{
	...
	{
		new Experiment4().testQuicksort();	// initiates the tests
	}
	
	/* Class methods & variables */
	long comparisonCounter;	// counts number of comparisons
	int cutoff; // cutoff value for when insertion sort becomes faster
	...
	public void testQuicksort()
	{
		...
		int startArraySize1 = 1000000;
		int endArraySize1 = 1000000;

		System.out.println("Array Size: \tCutoff: \tAverage Comparisons: \tExecution Time (milliseconds): "); // print data labels
			...
			for(int arraySize = startArraySize1; arraySize < endArraySize1+1; arraySize++) // for each array size
			{
				totalComparisons = 0;
					...
					for(int trials = 0; trials < numTrials; trials++)	// run quicksort 'numTrials' times
					{
						...
						startTime = System.currentTimeMillis()*1000;	// get start time
						
						quickSort(randomData); 		//quicksort array
						
						endTime = System.currentTimeMillis()*1000;	// get end time
					...
					executionTime = (endTime - startTime)/numTrials;	// calculate execution time 
					
					// print out array size and average comparisons
					System.out.println((arraySize) + "\t\t" + cutoff + "\t\t" + totalComparisons/numTrials + "\t\t\t" + executionTime);
			}
			cutoff++;
		}
	...
	{
		iSort(data, 1, data.length);	// call the recursive quicksort method
	}
	
	/**
	 * This method runs insertion sort
	 * 
	 * @param a The array to be sorted
	 */
	public void iSort(double[] a, int start, int end)
	{
		for(int i = start; i <= end; i++)		// used to be int i = 1; i < a.length
		{
			double temp = a[i];
			int j = i - 1;
			
			while(advanceCount((j >= 0) && (a[j] > temp)))	
			{
				a[j + 1] = a[j];
				j--;
			}
